...
class ProductType extends CI_Controller {
	...
	{	
		$this->load->view('bargain/admin/producttype', $this->data);
	}

	public function get(){
		$ptype=$this->producttype->get();
    	...
    	$i=1;
		foreach ($ptype as $junk) {
			$tr.="<tr>
					<th scope='row'>".$i."</th>
					<td>".$junk->name."</td>
					<td><button class='btn btn-sm btn-warning warning_33' data-toggle='modal' data-target='#editCategory' onclick='ediProductType(".$junk->id.")'>Edit</button></td>
					<td><button class='btn btn-danger' onclick='deleteProductType(".$junk->id.")'>Delete</button></td>
					";
					$i++;
		}
		echo $tr;
  ...
  //   </form>
	...
	$producttype=$this->producttype->get_by_id($_GET['id']);
	// print_r($category);
	$form="<form>
			<div class='form-group has-warning'>
		   	<input type='hidden' id='producttype_id'  class='form-control1' value='".$_GET['id']."' id='inputWarning1'>
		   	<input type='text' id='edit_producttype' class='form-control1' value='".$producttype['name']."' id='inputWarning1'>
			</div>
			";
			echo $form;
  ...
  //   </form>
	...
	function edit(){
		$id=$_GET['id'];
		$name=$_GET['name'];
		if($this->producttype->edit($id,$name)){

				$category=$this->producttype->get();
		// echo $this->db->last_query();
		// print_r($category);
		// echo count($category);
		$tr='';
	...
	function edit_sub_cat(){
		...
		foreach ($category as $junk) {
			$tr.="<tr>
					...
					<td>".$junk->parent_category."</td>
					<td>".$junk->sub_category."</td>
					<td><button class='btn btn-sm btn-warning warning_33' data-toggle='modal' data-target='#editSubCategory' onclick='ediProductType(".$junk->id.")' >Edit</button></td>
					<td><button class='btn btn-danger' onclick='deleteProductType(".$junk->id.")'>Delete</button></td>
					";
					$i++;
		}
	...
	function add(){
		$name=$_GET['name'];
		$this->producttype->add($name);
		$producttype=$this->producttype->get();
		// echo $this->db->last_query();
		// print_r($category);
		// echo count($category);
		$tr='';
